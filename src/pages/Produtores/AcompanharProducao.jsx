...
//   // Carrega produtores quando muda o estado
//   useEffect(() => {
...
//         p.nomeMunicipio === cidadeSelecionada
//       );
//     }
    
//     if (nomeProdutor) {
//       resultado = resultado.filter(p =>
//         p.nomeCompleto.toLowerCase().includes(nomeProdutor.toLowerCase())
//       );
//     }

//     return resultado;
//   }, [dados, cidadeSelecionada, nomeProdutor]);
...
//       // Atualização
//       setDados(prev => 
//         prev.map(p => p.id === produtorAtualizadoOuId.id ? produtorAtualizadoOuId : p)
//       );
//     } else {
//       // Exclusão
//       setDados(prev => prev.filter(p => p.id !== produtorAtualizadoOuId));
//     }
//   };

//   // Lista de cidades disponíveis (derivada dos dados)
//   const cidadesDisponiveis = useMemo(() => (
//     [...new Set(dados.map(p => p.nomeMunicipio))].sort()
//   ), [dados]);
...
//   return (
//     <>
//       {/* Cabeçalho e seleção de estado (mantido igual) */}
...
//         <p className="mt-2 font-bold text-xl">Cidades - {estadoSelecionado}</p>
...
//             className="w-full pl-4 pr-10 py-2 rounded-lg border border-black focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
//           />
...
//       {cidadeSelecionada || nomeProdutor ? (
//         <TableProdutores
...
//             {nomeProdutor.trim() && (
//               <>Não foi encontrado nenhum produtor com o nome "<strong>{nomeProdutor}</strong>" </>
//             )}
//             {cidadeSelecionada && (
//               <>na cidade de <strong>{cidadeSelecionada}</strong> </>
//             )}
//             no estado {estadoSelecionado === "PB" ? "da Paraíba" : "do Rio Grande do Norte"}.
//           </p>
//           <button
//             onClick={limparPesquisa}
...
//               Encontrados <strong>{dadosFiltrados.length}</strong> resultado(s) para "<strong>{nomeProdutor}</strong>"
//             </p>
//           )}
//           <TableProdutores
//             dados={dadosFiltrados}
...
const AcompanharProducao = () => {
  // Estados principais
  const [dados, setDados] = useState([]);
  const [todosOsDados, setTodosOsDados] = useState([]); // Todos os produtores para pesquisa global
  const [cidadeSelecionada, setCidadeSelecionada] = useState("");
  const [estadoSelecionado, setEstadoSelecionado] = useState("PB");
  const [nomeProdutor, setNomeProdutor] = useState("");

  // Carrega todos os produtores na inicialização (para pesquisa global)
  useEffect(() => {
  ...
  const handleProdutorAtualizado = (produtorAtualizadoOuId) => {
    if (typeof produtorAtualizadoOuId === 'object') {
      ...
      setDados(prev => prev.filter(p => p.id !== produtorAtualizadoOuId));
      setTodosOsDados(prev => prev.filter(p => p.id !== produtorAtualizadoOuId));
    }
  };
